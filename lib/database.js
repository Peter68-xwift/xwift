...
export const UserModel = {
  ...
  async findUserById(id) {
    ...
    const allReferrals = await users
      ...
      .project({ _id: 1, fullName: 1, email: 1, username: 1 })
      .toArray();

    const referralIds = allReferrals.map((ref) => ref._id);

    // Step 2: Get packages for referrals
    const referralPackages = await purchases
      .find({
    ...
    // Step 3: Map packages to referral users
    const packageMap = {};
    referralPackages.forEach((pkg) => {
      const key = pkg.userId.toString();
      if (!packageMap[key]) packageMap[key] = [];
      packageMap[key].push(pkg);
    });

    const enrichedReferrals = allReferrals.map((ref) => ({
      ...ref,
      packagesSubscribed: packageMap[ref._id.toString()] || [],
    }));

    // Step 4: Get the main user
    const user = await users.findOne({ _id: userId });

    if (!user) return null;

    // Step 5: Attach enriched referrals
    user.referrals = enrichedReferrals;
