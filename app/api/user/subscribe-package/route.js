...
export async function POST(request) {
  try {
    ...
    const { packageId, paymentMethod, transactionMessage, phoneNumber } =
      await request.json();
    ...
    const existingSubscription = await db
      .collection("packageSubscriptions")
      .findOne({
    ...
    if (existingSubscription) {
      return NextResponse.json(
        {
          error:
            "You already have an active or pending subscription for this package",
        },
        { status: 400 }
      );
    }
    ...
    if (paymentMethod === "mpesa") {
      if (!transactionMessage || !phoneNumber) {
        return NextResponse.json(
          {
            error:
              "Transaction message and phone number are required for M-Pesa payment",
          },
          { status: 400 }
        );
      }
      ...
      return NextResponse.json({
        success: true,
        message:
          "Subscription request submitted successfully. Awaiting payment verification and admin approval.",
        subscriptionId: result.insertedId,
      });
    }
